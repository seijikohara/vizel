...
This Decision states that `@vizel/core` is transitive, yet the adapter manifests declared `@vizel/core` as a peer dependency. The mismatch let the "install one adapter is sufficient" invariant depend on a package manager's auto-install-peers behaviour. WI-10 promotes `@vizel/core` from `peerDependencies` to `dependencies` (keeping the `workspace:^` spec) in `@vizel/react`, `@vizel/vue`, and `@vizel/svelte`, which realigns the manifests with this Decision. `@tiptap/*` and the framework runtime remain genuine peer dependencies. `scripts/check-publishable.ts` now asserts that each adapter declares `@vizel/core` in `dependencies` and never in `peerDependencies`.

The same change pins `prosemirror-model` to `1.25.7` in the `pnpm-workspace.yaml` overrides block. The dependency tree previously resolved two copies (`1.25.4` for the root-pinned `@tiptap/pm@3.23.6` and `1.25.7` for the collaboration runtime `@tiptap/y-tiptap` and `y-prosemirror`). Two copies break ProseMirror's `instanceof` Schema and Node identity checks across `prosemirror-state`, `prosemirror-view`, and `prosemirror-transform`. Pinning to `1.25.7` unifies both trees on the newest patch without forcing the collaboration runtime to downgrade. `scripts/check-publishable.ts` asserts a single resolved `prosemirror-model` version.
