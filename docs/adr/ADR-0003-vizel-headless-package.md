...
The Phase-2 deliverable ships `dismissable` only. The remaining five
primitives the original decision lists — `combobox`, `popover`,
`focus-trap`, `floating`, and `keyboard` — move out of the v2.0.0
blocker set and into the v2.x roadmap.

The reasons for the scope reduction:

- `dismissable` collapses the 21 `document.addEventListener` violations
  the audit identified. Closing the violation set is the load-bearing
  outcome of Phase 2; the other five primitives address scaling
  problems that do not block the v2.0.0 release.
- Each of the remaining primitives carries non-trivial design surface
  (the `combobox` primitive in particular must absorb roving tabindex,
  typeahead matching, and ARIA wiring for both menu and listbox
  consumers). Shipping each one safely needs its own ADR and its own
  Playwright coverage; bundling them with v2.0.0 risks an
  under-tested release.
- The adapter code that the remaining primitives would absorb
  currently lives behind a controller boundary already (`@vizel/core`
  controllers own the listeners; the framework components only mount
  and unmount them). The architectural goal — no `document.*` calls
  in framework components — holds even before the extra primitives
  land.

The v2.x roadmap entries are:

| Primitive | Target | Notes |
|-----------|--------|-------|
| `combobox` | v2.x | Replaces the per-feature menu controllers in `@vizel/core/controllers/` once the API stabilises across slash menu, mention menu, node selector, and toolbar dropdown. |
| `popover` | v2.x | Lifts the floating-positioning + dismissal shell out of `@vizel/core/controllers/popover.ts` once `floating` lands. |
| `focus-trap` | v2.x | Folds the modal-form focus boundary used by `VizelFindReplace` and `VizelLinkEditor` into one primitive. |
| `floating` | v2.x | Wraps `@floating-ui/dom` behind a stable surface so adapters cannot import the upstream package directly. |
| `keyboard` | v2.x | Consolidates `kbdMatch`, list-navigation, and typeahead helpers used across the suggestion menus. |

The v2.0.0 release notes record this scope clearly so consumers do
not expect the full primitive catalogue from the original ADR.
