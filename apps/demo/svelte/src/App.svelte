...
// Feature toggles, all defaulting on so the demo opens fully featured.
let features = $state({
  toolbar: true,
  theme: true,
  autoSave: true,
...
<VizelThemeProvider defaultTheme="system" storageKey="vizel-theme">
  <div class="app">
    ...
    <section class="features-section">
      ...
      <div class="features-toggles">
        ...
        <label class="feature-toggle">
          ...
          <select class="feature-select" bind:value={flavorName}>
    ...
    <main class="main">
      {#if features.outline && editorRef}
        <aside class="editor-aside" aria-label="Document outline">
            ...
            <div class="panel-content">
              {#if activeTab === "markdown" && features.syncPanel}
                <textarea
                  ...
                  oninput={handleMarkdownChange}></textarea>
              {:else if activeTab === "json" && features.syncPanel}
                <textarea
                  class="panel-textarea"
                  value={jsonInput}
                  placeholder="Edit JSON here..."
                  oninput={handleJsonChange}></textarea>
                    ...
                    {#each commentManager.comments as comment (comment.id)}
                      ...
                      >
                        <div class="panel-item-header">
                          ...
                          <span class="panel-item-meta">
                            {comment.author ? comment.author + " · " : ""}{new Date(
                              comment.createdAt
                            ).toLocaleString()}
                          </span>
                        </div>
                            ...
                            {#each comment.replies as reply (reply.id)}
                              <div class="panel-reply">
                                <span class="panel-reply-meta">
                                  {reply.author ? reply.author + " · " : ""}{new Date(
                                    reply.createdAt
                                  ).toLocaleString()}
                                </span>
                                <div>{reply.text}</div>
