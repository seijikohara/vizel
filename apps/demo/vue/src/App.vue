...
// Feature toggles. Most default on so the demo opens fully featured;
// `staticView` defaults off because it duplicates the editor surface.
...
// `generateVizelStaticHtml`. Only computed while the toggle is on
// so the demo stays light when the feature is hidden. A token
// guards against stale async results overwriting newer renders.
const staticHtml = ref("");
const staticError = ref<string | null>(null);
const staticRequestState = { cancelled: false };
function cancelStaticRequest(): void {
  staticRequestState.cancelled = true;
}
watch(
  [() => features.staticView, markdownInput, flavor],
  async ([enabled, markdown, currentFlavor]) => {
    cancelStaticRequest();
    if (!enabled) {
      staticHtml.value = "";
    ...
    if (!markdown) return;
    staticRequestState.cancelled = false;
    const localState = staticRequestState;
    try {
      const html = await generateVizelStaticHtml({ markdown, flavor: currentFlavor });
      if (localState.cancelled) return;
      staticHtml.value = html;
      staticError.value = null;
    } catch (error) {
      if (localState.cancelled) return;
      staticHtml.value = "";
      staticError.value = error instanceof Error ? error.message : "Failed to render static HTML";
    }
  }
);
onBeforeUnmount(cancelStaticRequest);
    ...
    <div class="app">
      ...
      <section class="features-section">
        ...
        <div class="features-toggles">
          ...
          <label class="feature-toggle">
            <input type="checkbox" v-model="features.staticView" />
            <span class="feature-toggle-label">Static View</span>
          </label>
          <label class="feature-toggle">
      ...
      <main class="main">
        ...
        <div class="editor-section">
          <div class="editor-container">
            ...
            <section
              v-if="features.staticView"
              class="static-preview"
              aria-label="Static HTML preview (server-renderable)"
            >
              <header class="static-preview-header">
                <span class="static-preview-title">Static View (read-only)</span>
                <span class="static-preview-hint">
                  Rendered by <code>generateVizelStaticHtml</code>
                </span>
              </header>
              <p v-if="staticError" class="static-preview-error">{{ staticError }}</p>
              <!--
                eslint-disable-next-line vue/no-v-html -- HTML originates from
                `generateVizelStaticHtml`, which sanitises against the Vizel
                schema before returning. The string never contains
                consumer-supplied script content.
              -->
              <div v-else class="static-preview-body" v-html="staticHtml"></div>
            </section>
          </div>
        </div>
        <aside
